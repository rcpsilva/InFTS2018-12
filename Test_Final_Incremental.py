...
del_strategy = ['delelete_bad', 'nothing']


fdataname = 'final_incremental_2019-05-07.txt'

dfile = open(fdataname, 'a+')
dfile.write('series,algorithm,order,nsets,f_nsets,bound_type,partitioner,mod,del_strategy,window_size,pearson,spearman,RMSE,MAPE,U,time,translation_threshold\n')
                    ...
                    for m in mod:
                        for d in del_strategy:
                            for tr in tresholds:
                                ...
                                u_metric = 1/(np.mean(np.abs((y[0:-2] - y[2:]))) / np.mean(np.abs(y[1:] - forecasts[0:-1])))

                                # Save data
                                row = '{},FIFTS,{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}\n'.format(s, o, n, fts.nsets, bt, p, m, d, np.mean(current_window_size), corr_p[0], corr_s[0], rmse, mape, u_metric, elapsed_time, tr)
                                dfile.write(row)
dfile.close()
